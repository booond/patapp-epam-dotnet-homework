...
public class ProposalService(IUnitOfWork unitOfWork, IMapper mapper, IAzureBlobService blobService) : IProposalService
{
    ...
    /// <inheritdoc/>
    public async Task<DataPage<ProposalDto>> GetAllProposalsAsync(HttpRequest request, int? top, int? skip)
    {
        ...
        var proposals = await unitOfWork.Proposals.GetProposalsAsync(false, queryOptions, top, skip);
        return mapper.Map<DataPage<ProposalDto>>(proposals);
    }
